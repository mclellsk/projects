...
namespace retrorunner2
{
    class Player : Sprite
    {
        ...
        public void Update(GameTime gameTime, List<Object> tiles, Camera camera)
        {
            ...
            {
                if (checkCollision(tile._hitbox, new Rectangle((int)(_position.X), (int)(_position.Y + _colOffset.Y + _velocity.Y), _frameSize.X, _frameSize.Y)))
                {
                    if (tile._position.Y > _position.Y) //Above Tile Collision
                    {
                        _isFalling = false;
                        _velocity.Y = 0;
                        _position.Y = tile._hitbox.Top - _hitbox.Height;
                        break;
                    }
                }
                if (checkCollision(tile._hitbox, new Rectangle((int)(_position.X), (int)(_position.Y - _colOffset.Y + _velocity.Y), _frameSize.X, _frameSize.Y)))
                {
                    if (tile._position.Y < _position.Y) //Underneath Tile Collision
                    {
            ...
            foreach (Object tile in tiles)
            {
                if (checkCollision(tile._hitbox, new Rectangle((int)(_position.X + _colOffset.X + _velocity.X), (int)(_position.Y), _frameSize.X, _frameSize.Y)))
                {
                    if (tile._position.X > _position.X) //To the Left of the Tile
                    {
                        _velocity.X = 0;
                        _position.X = tile._hitbox.Left - _hitbox.Width - _colOffset.X;
                    }
                }
                if (checkCollision(tile._hitbox, new Rectangle((int)(_position.X - _colOffset.X + _velocity.X), (int)(_position.Y), _frameSize.X, _frameSize.Y)))
                {
                    if (tile._position.X < _position.X) //To the Right of the Tile
                    {
                        _velocity.X = 0;
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch, Vector2 camera)
        {
            ...
            if (Game1.dev)
            {
                Rectangle dev_BoundingBox = new Rectangle((int)_screenPosition.X, (int)_screenPosition.Y, _hitbox.Width, _hitbox.Height);
