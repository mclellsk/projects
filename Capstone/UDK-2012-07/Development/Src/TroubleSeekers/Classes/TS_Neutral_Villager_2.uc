class TS_Neutral_Villager_2 extends TS_Pawn;
...
function InitializeConversations()
{
	...
	//Conversation 1
	Conversations.AddItem(2);
	//Conversation 2
	Conversations.AddItem(3);
}
...
simulated function bool Died(Controller Killer, class<DamageType> damageType, Vector HitLocation)
{
	...
	MyPawn(Killer.Pawn).TSEventManager.AddKill(class'TS_Neutral_Villager_2');
...
{
	if (self.ConversationIndex == 0 && Target.TSConversationManager.CheckDecision(2, 2, 5))
	{
		self.ConversationIndex = 1;
	}
	if (self.ConversationIndex == 0 && Target.TSConversationManager.CheckDecision(2, 3, 7))
	{
		self.ConversationIndex = 1;
	}
	if (self.ConversationIndex == 0 && Target.TSConversationManager.CheckDecision(2, 7, 11))
	{
		self.ConversationIndex = 1;
	}
	if (self.ConversationIndex == 0 && Target.TSConversationManager.CheckDecision(2, 7, 12))
	{
		self.ConversationIndex = 1;
	}
