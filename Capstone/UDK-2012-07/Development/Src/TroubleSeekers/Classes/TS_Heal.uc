class TS_Heal extends TS_Projectile;
...
simulated function ProcessTouch(Actor Other, Vector HitLocation, Vector HitNormal)
{
	...
	isZombie = false;
	for (i = 0; i < TS_Pawn(Other).statusEffectsContainer.Length; i++)
	{
		if (TS_Pawn(Other).statusEffectsContainer[i] == isZombified)
		{
			isZombie = true;
			//Double Damage Multiplier for Holy
			Other.TakeDamage(Damage * 2,InstigatorController,HitLocation,MomentumTransfer * Normal(Velocity), MyDamageType,, self);
		}
	}
	if (!isZombie)
	{
...
DefaultProperties
{
	...
	ProjFlightTemplate=ParticleSystem'UPK_Props.Holy_Moving';
    ProjExplosionTemplate=ParticleSystem'UPK_Props.Holy_Explosion';
	SpawnSound=SoundCue'UPK_Sounds.Holy_Cue';
